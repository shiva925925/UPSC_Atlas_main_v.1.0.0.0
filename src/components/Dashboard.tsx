...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="p-6 animate-fade-in overflow-y-auto h-full bg-app-bg transition-colors duration-300">
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-text-main">Welcome back, {userProfile?.name || 'Aspirant'}!</h1>
        <p className="text-text-muted mt-2">You've studied for <span className="font-bold text-blue-600">{totalHours} hours</span> total. Keep pushing!</p>
      </header>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <GlassCard variant="opaque" className="p-6 flex items-center gap-4 border-card-border">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-text-main">{totalHours}h</h3>
          </div>
        </GlassCard>
        ...
        <GlassCard variant="opaque" className="p-6 flex items-center gap-4 border-card-border">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-text-main">{completedTasks}</h3>
          </div>
        </GlassCard>
        ...
        <GlassCard variant="opaque" className="p-6 flex items-center gap-4 border-card-border">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-text-main">{inProgressTasks}</h3>
          </div>
        </GlassCard>
        ...
        <GlassCard variant="opaque" className="p-6 flex items-center gap-4 border-card-border">
          ...
          <div>
            <p className="text-sm text-text-muted">Target Year</p>
            <h3 className="text-2xl font-bold text-text-main">{userProfile?.targetYear || 2025}</h3>
          </div>
        </GlassCard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
        <GlassCard variant="opaque" className="lg:col-span-2 p-6 border-card-border">
          <h3 className="text-lg font-bold text-text-main mb-4">Subject Distribution</h3>
              ...
              return Object.entries(categoryMinutes).map(([categoryKey, minutes]) => {
                ...
                return (
                  <div key={categoryEnum} className="flex flex-col items-center gap-2 w-full">
                    ...
                    ></div>
                    <span className="text-[10px] text-text-muted truncate w-full text-center">{categoryEnum}</span>
                  </div>
                );
              });
            })()}
        ...
        <GlassCard variant="opaque" className="p-6 border-card-border">
          <h3 className="text-lg font-bold text-text-main mb-4">Recent Logs</h3>
          <div className="space-y-4">
            {allLogs.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5).map(log => (
              <div key={log.id} className="flex items-center gap-3 pb-3 border-b border-card-border/30 last:border-0">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-text-main">{log.description}</p>
                  <p className="text-xs text-text-muted">{log.date} • {log.durationMinutes}m</p>
                </div>
              </div>
            ))}
            {allLogs.length === 0 && <p className="text-sm text-text-muted">No study time logged yet.</p>}
          </div>
        </GlassCard>
      </div>
    </div>
